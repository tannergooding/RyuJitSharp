...
public enum GenTreeFlags : uint
{
    ...
    GTF_ADDRMODE_NO_CSE = 0x80000000,

    /// <summary>GT_MUL     -- produce 64-bit result</summary>
    GTF_MUL_64RSLT = 0x40000000,

    /// <summary>GT_&lt;relop&gt; -- Is branch taken if ops are NaN?</summary>
    GTF_RELOP_NAN_UN = 0x80000000,

    /// <summary>GT_&lt;relop&gt; -- result of compare used for jump or ?: with explicit "loop test" in the header block.</summary>
    GTF_RELOP_JMP_USED = 0x40000000,

    /// <summary>GT_RETURN -- This is a return generated during epilog merging.</summary>
    GTF_RET_MERGED = 0x80000000,

    /// <summary>GT_BOX -- this box and its operand has been cloned, cannot assume it to be single-use anymore</summary>
    GTF_BOX_CLONED = 0x40000000,

    /// <summary>GT_BOX -- "box" is on a value type</summary>
    GTF_BOX_VALUE = 0x80000000,

    /// <summary>GT_QMARK -- early expansion of the QMARK node is required</summary>
    GTF_QMARK_EARLY_EXPAND = 0x01000000,

    /// <summary>GT_ARR_ADDR -- this array's address is not null</summary>
    GTF_ARR_ADDR_NONNULL = 0x80000000,

    /// <summary>// 0xFF000000, bits used by the handle types.</summary>
    GTF_ICON_HDL_MASK = (~0u) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_SCOPE_HDL = (HandleKindIndex.GTF_ICON_SCOPE_HDL + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_CLASS_HDL = (HandleKindIndex.GTF_ICON_CLASS_HDL + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_METHOD_HDL = (HandleKindIndex.GTF_ICON_METHOD_HDL + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_FIELD_HDL = (HandleKindIndex.GTF_ICON_FIELD_HDL + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_STATIC_HDL = (HandleKindIndex.GTF_ICON_STATIC_HDL + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_STR_HDL = (HandleKindIndex.GTF_ICON_STR_HDL + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_OBJ_HDL = (HandleKindIndex.GTF_ICON_OBJ_HDL + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_CONST_PTR = (HandleKindIndex.GTF_ICON_CONST_PTR + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_GLOBAL_PTR = (HandleKindIndex.GTF_ICON_GLOBAL_PTR + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_VARG_HDL = (HandleKindIndex.GTF_ICON_VARG_HDL + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_PINVKI_HDL = (HandleKindIndex.GTF_ICON_PINVKI_HDL + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_TOKEN_HDL = (HandleKindIndex.GTF_ICON_TOKEN_HDL + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_TLS_HDL = (HandleKindIndex.GTF_ICON_TLS_HDL + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_FTN_ADDR = (HandleKindIndex.GTF_ICON_FTN_ADDR + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_CIDMID_HDL = (HandleKindIndex.GTF_ICON_CIDMID_HDL + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_BBC_PTR = (HandleKindIndex.GTF_ICON_BBC_PTR + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_STATIC_BOX_PTR = (HandleKindIndex.GTF_ICON_STATIC_BOX_PTR + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_FIELD_SEQ = (HandleKindIndex.GTF_ICON_FIELD_SEQ + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_STATIC_ADDR_PTR = (HandleKindIndex.GTF_ICON_STATIC_ADDR_PTR + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_SECREL_OFFSET = (HandleKindIndex.GTF_ICON_SECREL_OFFSET + 1) << HANDLE_KIND_INDEX_SHIFT,

    GTF_ICON_TLSGD_OFFSET = (HandleKindIndex.GTF_ICON_TLSGD_OFFSET + 1) << HANDLE_KIND_INDEX_SHIFT,

    // /// <summary>GT_CNS_INT -- GTF_REUSE_REG_VAL, defined above</summary>
    // GTF_ICON_REUSE_REG_VAL = 0x00800000,

    /// <summary>GT_CNS_INT -- constant is Vector&lt;T&gt;.Count</summary>
    GTF_ICON_SIMD_COUNT = 0x00200000,
